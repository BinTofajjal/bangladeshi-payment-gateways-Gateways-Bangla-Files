 ...
 * @since   1.0.0
 */
class Rocket extends \WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->pg_icon = apply_filters( 'bdpg_rocket_icon', BD_PAYMENT_GATEWAYS_DIR_URL . '/assets/images/Rocket.png' );
		...
		add_action( 'woocommerce_thankyou_woo_rocket', array( $this, 'bdpg_thankyou' ) );
		...
		if ( isset( $rocket_settings['rocket_charge'] ) && 'yes' === $rocket_settings['rocket_charge'] ) {
	...
	public function payment_fields() {
		...
		$total_amount = '◉ আপনাকে মোট পাঠাতে হবে ⎯ <b style="font-size:18px;font-weight:500; color:blue">'. $woocommerce->cart->total . '</b> ‍‍‍​‍‍‍টাকা।</br>';
		...
		foreach ( $this->accounts as $account ) {
			...
			?>
				<div class="bdpg-acc_d">
					<p>অ্যাকাউন্ট ধরন: <b style="font-weight:600"><?php echo esc_html( $account['type'] ); ?></b></p>
					<p>অ্যাকাউন্ট নাম্বার: <b style="font-weight:600"><?php echo esc_html( $account['number'] ); ?></b> </p>
				</div>
			</div>
			<?php
		}
	...
	public function process_payment( $order_id ) {
		...
		$order->update_status( 'on-hold', __( 'Awaiting Rocket payment', 'bangladeshi-payment-gateways' ) );
	...
	public function payment_process() {
		if ( 'woo_rocket' !== $_POST['payment_method'] ) {
			return;
		}
	...
	public function admin_order_data( $order ) {
		...
		<div class="form-field form-field-wide bdpg-admin-data">
			<img src="<?php echo esc_url( $this->pg_icon ); ?> " alt="Rocket">
	...
	public function data_order_review_page( $order ) {
		...
		<div class="bdpg-g-details">
			<img src="<?php echo esc_html( $this->pg_icon ); ?> " alt="Rocket">
			<table class="wp-list-table widefat striped posts">
				<tbody>
					<tr>
						<th>
							<strong>
								<?php echo esc_html__( 'Rocket Number', 'bangladeshi-payment-gateways' ); ?>
							</strong>
						</th>
						<td>
		<?php echo esc_attr( $number ); ?>
	...
	public function admin_column_value( $column ) {
		...
		$payment_no = ( get_post_meta( $post->ID, 'woo_rocket_number', true ) ) ? get_post_meta( $post->ID, 'woo_rocket_number', true ) : '';
		$tran_id    = ( get_post_meta( $post->ID, 'woo_rocket_trans_id', true ) ) ? get_post_meta( $post->ID, 'woo_rocket_trans_id', true ) : '';
